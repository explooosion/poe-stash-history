...
import { AiOutlineFire, AiOutlineTeam, AiOutlineSketch, AiOutlineRocket, AiOutlineDeploymentUnit } from 'react-icons/ai';
...
function Dashboard() {
  ...
  const avgLevel = _.round(_.chain(currentLeagueMembers)
  ...
  const classGroup = _.chain(currentLeagueMembers)
    ...
    .flattenDeep()
    .map(mb => ({
      class: _.get(classes.find(c => c.class === mb.class), 'name'),
      classId: _.get(classIds.find(c => c.classId === mb.classId), 'name'),
    }))
    .groupBy('class')
  ...
  const Cards = [
    ...
    { id: 2, title: '幹部人數', text: String(officerTotal), icon: AiOutlineSketch },
    { id: 3, title: '成員人數', text: String(memberTotal), icon: AiOutlineTeam },
    { id: 4, title: '公開帳號', text: String(publicTotal), icon: AiOutlineRocket },
    { id: 5, title: '當季平均等級', text: String(avgLevel), icon: AiOutlineDeploymentUnit },
  ]
  ...
  return (
    <Main>
      ...
      <div className="card" style={{ margin: '3rem' }}>
        <PieChart type="doughnut" datas={classIdGroup} title="當季聯盟角色統計圖" />
