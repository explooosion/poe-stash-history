...
export function replaceUrlParams(url = '', params = {}) {
  if (_.isEmpty(params)) {
    ...
    const mapObj = _.chain(params).keys(params).reduce((acc, cur) => ({ ...acc, [`${prefix}${cur}`]: params[cur] }), {}).value();
    // /:id|:name/gi
    const rexStr = _.chain(mapObj).keys(mapObj).reduce((acc, cur) => [...acc, cur], []).join('|').value();
